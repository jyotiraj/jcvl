 ...
 *
 * Parameters v0.5.8
 *   selectParents  - when true selects all the parents of the child node in the hierarchy above. False => only current node is selected
 ...
 * License:  BSD
 *
 * Author:   Jyoti Raj <jyotiraj@gmail.com>
 * Version:  0.5.8
 * Started:  15.04.2014
 * Modified: 22.04.2014
 */
...
function jCVL_Label(opts) {
    ...
    this.description = opts.description || this.text;

    var clSel = 'cvl-selected-label',
        clDel = 'cvl-selected-del',
        clName = 'cvl-selected-name';
    ...
    var nameElem = $('<span>')
        ...
        .html(this.text)
        .click(function(ev) {
            that.doOnNameClick(ev);
        });
    var valElem = $('<input type="hidden">')
        .attr('class', this.paramName)
...
// Sets value
jCVL_Label.prototype.setDescription = function(description) {
    this.elems.valElem.attr('description', this.description = description);
}
...
// Returns a description from label
jCVL_Label.prototype.getDescription = function() {
...
jCVL_Label.prototype.remove = function(cb) {
    var that = this;
    this.elems.elem.fadeOut(200, function() {
        that.get().remove();
        if (cb) cb();
    });
}
...
jCVL_LabelArea.prototype.addLabel = function(text, value, description) {
    var val = value || text;
    ...
    if (!this.unique || !this.hasValue(val)) {
        ...
        j.setOnDelClick(function(ev, ev_id, ev_text, ev_value, ev_description) {
            that.onLabelDelClick(ev, ev_id, ev_text, ev_value, ev_description, j);
        });
        j.setOnNameClick(function(ev, ev_id, ev_text, ev_value, ev_description) {
            that.onLabelNameClick(ev, ev_id, ev_text, ev_value, ev_description, j);
        });
        this.jaws.push(j);
...
jCVL_LabelArea.prototype.onLabelDelClick = function(ev, id, text, value, description, jaw) {
    var that = this;
    this.delLabel(value, function() {
        that.onDelClick(ev, id, text, value, description);
    });
}

// onClick handler for name element of each label
jCVL_LabelArea.prototype.onLabelNameClick = function(ev, id, text, value, description, jaw) {
    this.onNameClick(ev, id, text, value, description);
}
...
// Returns text label of item
jCVL_ColumnItem.prototype.getDescription = function() {
...
// Sets element's description
jCVL_ColumnItem.prototype.setDescription = function(desc) {
...
jCVL_ColumnItem.prototype.getIndexPath = function() {
    ...
    while (it = it.getParentColumn().getParentItem());
    return p.reverse();
}
...
function jCVL_Column(opts) {
    ...
    this.parentDescription = this.parentItem ? this.parentItem.getDescription() : undefined;
    ...
    this.waiter = new jCVL_ColumnWaiter({
        className: this.opts.ajaxSource.waiterClassName
    });
...
jCVL_Column.prototype._createItem = function(index, text, value, description) {
    ...
    var that = this;
    item.setOnClick(function(ev, item) {
        that.onItemClick(ev, index, item);
    });
    item.setOnCheckboxClick(function(ev, item) {
        that.onItemCheckboxClick(ev, index, item);
    });
    return item;
}
...
jCVL_Column.prototype._fillItems = function(data) {
    ...
    jQuery.each(data, function(index, d) {
        var item = that._createItem(index, d.name, d.value, d.description);
...
jCVL_Column.prototype.setData = function(data, parentItem) {
    ...
    this.parentDescription = (parentItem) ? parentItem.getDescription() : undefined;
...
jCVL_Column.prototype.getSelectedItem = function() {
    return this.curItem;
}
jCVL_Column.prototype.getSelectedIndex = function() {
...
jCVL_Column.prototype.getParentItem = function() {
    return this.parentItem;
}
jCVL_Column.prototype.getParentText = function() {
    return this.parentText;
}
jCVL_Column.prototype.getParentDescription = function() {
    return this.parentDescription;
}
jCVL_Column.prototype.getParentValue = function() {
...
jCVL_Column.prototype.setParentItem = function(item) {
    if (item) {
        ...
        this.parentDescription = item.getDescription();
...
jCVL_Column.prototype.getItemsString = function(bTextOrValue) {
    ...
    jQuery.each(this.items, function(index, item) {
        str.push( !! bTextOrValue ? item.getText() : item.getValue());
    });
    return str;
}
...
jCVL_Column.prototype.getFullPath = function(index, toLevel_CurrentIndex, toLevel_ColumnIndex) {
    ...
    if (index >= 0 && index < this.items.length) {
        ...
        str.push({
            text: curIt.getText(),
            value: curIt.getValue(),
            description: curIt.getDescription(),
            hasChildren: this.itemHasChildren(index)
        });
        ...
        while (p && (curLevel == -1 || --curLevel >= toLevel)) {
            ...
            str.push({
                ...
                hasChildren: pCol.itemHasChildren(p.getIndex())
            });
...
jCVL_Column.prototype.hasChildren = function() {
    ...
    var that = this;
    jQuery.each(this.items, function(index, item) {
        if (that.data[index].hasChildren) has = true;
    });
    return has;
}
...
jCVL_Column.prototype.addItem = function(item, pos) {
    ...
    var index = pos !== undefined && parseInt(pos) >= 0 && parseInt(pos) < this.items.length ? parseInt(pos) : this.items.length;
    this.data.splice(index, 0, it);
    this.refresh();
}

jCVL_Column.prototype.removeItem = function(pos) {
    var index = pos !== undefined && parseInt(pos) >= 0 && parseInt(pos) < this.items.length ? parseInt(pos) : -1;
    if (index > -1) {
...
jCVL_Column.prototype.setItemData = function(index, data) {
    var pos = index !== undefined && parseInt(index) >= 0 && parseInt(index) < this.data.length ? parseInt(index) : -1;
    if (pos > -1) {
...
jCVL_ColumnSplitter.prototype._unbindMouseEvents = function() {
    ...
    this.parentEl
        .unbind('mousemove', function(event) {
            that.onMouseMove(event);
        })
        .unbind('mouseleave', function(event) {
            that.onMouseUpOut(event);
        })
...
jCVL_ColumnList.prototype.adjustElements = function() {
    ...
    if (this.opts.useSplitters)
        jQuery.each(this.spls, function(i, s) {
            s.adjustHeight();
        });
}
            ...
            nextCol.show(function() {
                ...
                if (that.wrapper.width() < that._calculateWidth())
                    that.wrapper.animate({
                        scrollLeft: that._calculateWidth() - that.wrapper.width()
                    }, 'fast');
            });
...
jCVL_ColumnList.prototype.setData = function(data, columnNum) {
    var cnum = arguments.length > 1 && parseInt(columnNum) >= 0 && parseInt(columnNum) < this.opts.columnNum ? parseInt(columnNum) : 0;
    if (cnum == 0)
        this.clear();
...
jCVL_ColumnList.prototype.onColumnItemCheckboxClick = function(ev, colIndex, itemIndex, item) {
    ...
    if (item.isChecked()) {
        ...
        try {
            ...
            err = e;
        } // to avoid errors occured in callbacks

        if (item == this.getColumn(colIndex).getSelectedItem()) {
            // if select parents was On then uncheck all children else only uncheck current elemvent
            if (this.opts.selectParents) {
...
function jCVL_ColumnListView(opts) {
    ...
    listOpts.onClick = function(ev, ci, ii, it) {
        that.onColumnItemClick(ev, ci, ii, it);
    };
    listOpts.onCheckboxClick = function(ev, ci, ii, it) {
        that.onColumnItemCheckboxClick(ev, ci, ii, it);
    };
    ...
    this.jaws = new jCVL_LabelArea({
        ...
        onDelClick: function(ev, id, text, value, description) {
            that.onLabelDelClick(ev, id, text, value, description);
        },
        onNameClick: function(ev, id, text, value, description) {
            that.onLabelNameClick(ev, id, text, value, description);
        }
    });
    this.labels = {};
...
jCVL_ColumnListView.prototype.setFromURL = function(in_url, col_num) {
    var cnum = arguments.length > 1 && parseInt(col_num) >= 0 && parseInt(col_num) < this.opts.columnNum ? parseInt(col_num) : 0;
    var that = this;
...
jCVL_ColumnListView.prototype._checkULElement = function(elem) {
    var ul = typeof(elem) == 'string' ? $((elem.charAt(0) == '#' ? '' : '#') + elem) : $(elem);
    if (ul.length == 0)
...
jCVL_ColumnListView.prototype._parseData = function(ul_elem, data) {
    ...
    $(ul_elem).children('li').each(function(index, item) {
        ...
        var text_item = $.trim($($(item).contents()[0]).text());
        var name = ulChildren.length ? text_item : item.innerHTML; //$.trim($($(item).contents()[0]).text());
        var value = $(item).attr("data-value") || text_item; //name;
        var description = $(item).attr("data-description") || text_item; //name;
...
jCVL_ColumnListView.prototype.onColumnItemCheckboxClick = function(event, colIndex, itemIndex, item) {
    ...
    if (item.isChecked()) {
        ...
        //if parents selection is turned off - then just get the child element
        if (!this.opts.selectParents) {
            labs = labs[labs.length - 1];
            if (typeof(this.labels[labs.value]) == 'undefined') {
                this.labels[labs.value] = 1;
                this.jaws.addLabel(labs.text, labs.value, labs.description);
            } else {
                this.labels[item.value]++;
            }
        } else {
            ...
            jQuery.each(labs, function(index, item) {
                if (typeof(that.labels[item.value]) == 'undefined') {
                    ...
                    if (!that.opts.leafMode || !item.hasChildren) {
                        that.jaws.addLabel(item.text, item.value, item.description);
                    }
                } else {
...
jCVL_ColumnListView.prototype.onLabelDelClick = function(event, id, text, value, description) {
...
jCVL_ColumnListView.prototype.onLabelNameClick = function(event, id, text, value, description) {
    var ptc = this._findListItemPath(this.list.getData(), value);
    if (ptc.length)
